 ...
 *
 * Created by zjm on 2019/7/9 17:40 PM
 */
public class RLEIterator {
    ...
    public RLEIterator(int[] A) {
        ...
        for(int i = 0; i < A.length; i += 2) {
            if(A[i] != 0) {
                if(map.size() < 1) {
                    map.put((long) A[i], A[i+1]);
                }else {
                    map.put(A[i] + map.lastKey(), A[i+1]);
                }
            }
        }
    }
    ...
    public int next(int n) {
        ...
        count += n;
        if(count > map.lastKey()) {
            return -1;
        }
        return map.get(map.ceilingKey(count));
    }
    ...
    class RLEIterator2 {
        ...
        public int next(int n) {
            ...
            count += n;
            while(index < a.length && count > a[index]) {
                count -= a[index];
                index += 2;
            }
            if(index >= a.length) {
