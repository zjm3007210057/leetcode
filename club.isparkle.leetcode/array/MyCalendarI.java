 ...
 * <p>
 * Created by zjm on 2019/6/29 19:40
 */
public class MyCalendarI {
    ...
    public boolean book(int start, int end) {
        Integer smallKey = map.floorKey(end-1);
        Integer largeKey = map.ceilingKey(start);
        if(smallKey != null && map.get(smallKey) > start) {
            return false;
        }
        if(largeKey != null && largeKey < end) {
            return false;
        }
        map.put(start, end);
        return true;
